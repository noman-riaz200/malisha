...
export function FindUniversitySection() {
  ...
  return (
    <section suppressHydrationWarning className="py-16 bg-white">
      <div suppressHydrationWarning className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div suppressHydrationWarning className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          {largeImages.map((uni) => (
            <Link
            ...
            >
              <div suppressHydrationWarning className="relative h-48 md:h-56 lg:h-64">
                <Image
                  src={uni.image}
                  alt={uni.name}
                  fill
                  ...
                  suppressHydrationWarning
                />
                <div suppressHydrationWarning className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                <div suppressHydrationWarning className="absolute bottom-0 left-0 right-0 p-4 md:p-6 flex items-center justify-center">
                  <h3 suppressHydrationWarning className="text-lg md:text-xl font-bold text-white tracking-wide">
                    {uni.name}
                  </h3>
                </div>
              </div>
        ...
        <div suppressHydrationWarning className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {smallImages.map((uni) => (
            <Link
              suppressHydrationWarning
              key={uni.id}
              ...
              className="group relative block overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-all duration-300"
            >
              <div suppressHydrationWarning className="relative h-28 sm:h-32 md:h-36">
                <Image
                  src={uni.image}
                  alt={uni.name}
                  fill
                ...
                />
                <div suppressHydrationWarning className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
                <div suppressHydrationWarning className="absolute bottom-0 left-0 right-0 p-2 md:p-3 flex items-center justify-center">
                  <h3 suppressHydrationWarning className="text-xs md:text-sm font-bold text-white tracking-wide text-center">
                    {uni.name}
                  </h3>
                </div>
              </div>
